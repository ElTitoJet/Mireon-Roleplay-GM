...
-- Funciones Principales
    function FamCommands(source, command)
        local Permiso = FamCheckRank(source)
        if not Permiso then
            return
        end
        outputChatBox("#9AA498==== [#A03535Lista de Comandos Familia#9AA498] ====", source, 255, 255, 255, true)
    ...
    function FamStart()
        ...
        setTimer(function()
            local varDataFamilias = exports["MR1_Inicio"]:query('SELECT * FROM Familias WHERE Activo = "true";')
            for i, Familia in ipairs(varDataFamilias) do
                -- VEHICULOS
    ...
    function setFamUser(client)
        ...
        if varFamiliasUsers and #varFamiliasUsers >= 1 then
            local varDataFamilias = exports["MR1_Inicio"]:query('SELECT * FROM Familias WHERE ID = ?', varFamiliasUsers[1].Familia)
            Familia = {
                FamiliaID = varFamiliasUsers[1].Familia,
                Nombre = varDataFamilias[1].Nombre,
    ...
    function famasignar(source, cmd, objetivoID, IDFam, IDRango)
        ...
        -- Actualizar los datos del jugador objetivo en la DB
            local varDataFamilias = exports["MR1_Inicio"]:query("SELECT * FROM FamiliasUsers WHERE Personajes=?", varDataUser.IDPersonaje)
            if varDataFamilias and #varDataFamilias == 1 then
    ...
    function enviarTerritoriosFamilias()
        ...
        -- Enviar la tabla ya procesada al cliente
            ...
            --triggerClientEvent(source, "Familias::Territorios::RecibirTerritorios", source, varDataFamilias)
    end
    ...
    function FamVehExplote()
        -- Obtenemos la data del vehiculo
            ...
            local varDataFamilias = exports["MR1_Inicio"]:query('SELECT * FROM Familias WHERE ID = "?"', varDataFamVehicles[1].IDFamilia)
            ...
            setTimer(function()
                for i2, Vehiculo in ipairs(varDataFamVehicles) do
                    ...
                    if not isVehicleExists(vehInformacion["Matricula"]) then
                        ...
                        exports["MR1_Inicio"]:setValue(veh, 'Personaje', varDataFamilias[1].Nombre)
                        exports["MR1_Inicio"]:setValue(veh, 'Informacion', vehInformacion)
                        exports["MR1_Inicio"]:setValue(veh, 'Estado', vehEstado)
                        exports["MR1_Inicio"]:setValue(veh, 'Inventario', vehInventario)
                        setElementHealth( veh, tonumber(vehEstado["Salud"]))
...
-- Eventos y Handlers
    ...
    addCommandHandler("fam", FamCommands)
...
-- Por ejemplo, spawnear vehículos, etc.
    addEventHandler("onResourceStart", resourceRoot, FamStart)
